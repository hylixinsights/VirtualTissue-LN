...
function updateScene(){
 ...
 cells.forEach((c,i)=>{
  ...
  temp.position.set(c.x,0,-c.y);temp.rotation.set(0,0,0);temp.scale.set(radius, radius*scaleY,radius*(c.kind==='DC'?1.18:1));temp.updateMatrix();bodies.setMatrixAt(i,temp.matrix);
  ...
  temp.position.set(c.x-.5,radius*scaleY*.58,-c.y+.45);temp.scale.set(radius*.58,radius*scaleY*.5,radius*.54);temp.updateMatrix();nuclei.setMatrixAt(i,temp.matrix);nuclei.setColorAt(i,color.clone().multiplyScalar(.57));
  if(['FDC','FRC','DC','MAC'].includes(c.kind))for(let k=0;k<(c.kind==='FDC'?7:4);k++){
 ...
 });bodies.instanceMatrix.needsUpdate=true;bodies.instanceColor.needsUpdate=true;nuclei.instanceMatrix.needsUpdate=true;nuclei.instanceColor.needsUpdate=true;
...
function labels(){if(tissue){const p0=new THREE.Vector3(0,0,0).project(camera),p1=new THREE.Vector3(50*150/tissue.params.radius_um,0,0).project(camera);$('.scale i').style.width=Math.abs(p1.x-p0.x)*canvas.clientWidth/2+'px';}const w=canvas.clientWidth,h=canvas.clientHeight;for(const el of document.querySelectorAll('[data-zone]')){const key=el.dataset.zone;el.hidden=!showZones||(key==='dz'||key==='lz')&&!tissue?.gc;const p=new THREE.Vector3(...labelPositions[key]).project(camera);el.style.left=`${(p.x*.5+.5)*w}px`;el.style.top=`${(-p.y*.5+.5)*h}px`;}}
let lastSize='';function render(){requestAnimationFrame(render);const w=canvas.clientWidth,h=canvas.clientHeight,key=w+','+h;if(key!==lastSize){renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();lastSize=key;}controls.update();renderer.render(scene,camera);labels();}
render();
let pointerStart;canvas.addEventListener('pointerdown',e=>{pointerStart=[e.clientX,e.clientY];});canvas.addEventListener('pointerup',e=>{if(!pointerStart||Math.hypot(e.clientX-pointerStart[0],e.clientY-pointerStart[1])>5||!bodies)return;const rect=canvas.getBoundingClientRect(),ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1),camera);const hits=ray.intersectObject(bodies);if(hits.length){selected=cells[hits[0].instanceId].id;inspect();updateScene();}});
...
function update(value,append=true){data=value;token=value.token||token;tissue=value.tissue;if(append&&!recording){const f={time_min:tissue.time_min,metrics:tissue.metrics};if(history.at(-1)?.time_min!==f.time_min)history.push(f);}
 ...
 $('#legend').innerHTML=Object.entries(data.registry).map(([kind,r])=>`<button class="legend-row ${filter&&filter!==kind?'dim':''}" data-kind="${kind}"><span class="dot" style="background:${r.color}"></span>${r.label}<b>${m.types[kind]||0}</b></button>`).join('');document.querySelectorAll('[data-kind]').forEach(el=>el.onclick=()=>{filter=filter===el.dataset.kind?null:el.dataset.kind;$('#clearFilter').hidden=!filter;updateScene();update(data,false);});
 ...
 updateScene();inspect();chart();if(tissue.stop_reason){running=false;notice(tissue.stop_reason);}
 controlsUI();
}
function controlsUI(){const offline=!!recording;$('#play').textContent=running?'Ⅱ Pause':offline?'▶ Play recording':data?.provider.provider==='jev'?'▶ Run with Jev':'▶ Run demonstration';$('#step').disabled=busy||running||offline;$('#pulse').disabled=busy||running||offline||tissue?.scenario==='baseline';$('#reset').disabled=busy||running||offline;$('#scenario').disabled=running||busy||offline;$('#population').disabled=running||busy||offline;$('#seed').disabled=running||busy||offline;$('#provider').disabled=running||busy||offline;$('#export').disabled=busy||offline;}
...
$('#seek').oninput=e=>showFrame(+e.target.value);$('#leaveReplay').onclick=async()=>{running=false;recording=null;replayMilestones();$('#replay').hidden=true;update(await api('/api/state'),false);$('#provider').value=data.provider.provider;$('#population').value=tissue.metrics.initial;notice('Returned to the current live tissue.');};
try{update(await api('/api/state'));$('#population').value=tissue.metrics.initial;$('#setupHint').textContent=`Starts with ${tissue.metrics.initial} cells and no germinal center.`;$('#loading').hidden=true;await refreshRecordings();const saved=new URLSearchParams(location.search).get('recording');if(saved)await openSavedRecording(saved);}catch(e){$('#loading').textContent=e.message;notice(e.message,true);}
