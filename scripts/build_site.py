...
'''
 ...
 (site/'web/app.mjs').write_text(js)
 html=(ROOT/'web/index.html').read_text().replace('"/web/','"./web/').replace('"/vendor/','"./vendor/')
 ...
 import html as h
 manual=(ROOT/'docs/BIOLOGICAL_MANUAL.md').read_text();scope=(ROOT/'docs/IMPLEMENTATION.md').read_text()
 ...
 (site/'model.html').write_text(model+'<details open><summary>Implemented scope</summary><pre>'+h.escape(scope)+'</pre></details><details><summary>Original biological manual</summary><pre>'+h.escape(manual)+'</pre></details></html>')
